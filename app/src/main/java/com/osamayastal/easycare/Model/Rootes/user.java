...
public class user {
    ...
    public void Post_resend(final Context mcontext, final String user_id, final user_Listener lisenner){
        if (queue==null) {
            queue = Volley.newRequestQueue(mcontext);
        }
        final ProgressDialog dialog=new AppPop().Loading_POP(mcontext,mcontext.getString(R.string.resend_conf_code));

        String url = Server_info.API+"api/mobile/Resend";;
        lisenner.onStart();
        StringRequest postRequest = new StringRequest(Request.Method.POST, url,
                new Response.Listener<String>()
                {
